...
export const createGuestOrderController = async (req, res) => {
  try {
    const { products, totalcost, mplan } = req.body;
    ...
    // Create a new order without a buyer
    const order = new orderModel({
      products,
      buyer: null, // Guest orders have no buyer
      totalcost,
      mplan,
      status: "Not Process",
    });
    ...
    res.status(500).send({
      success: false,
      error,
      message: "Error in creating guest order",
    });
  }
};
